...
const Main = () => {
    ...
    return (
        <div className='w-full mt-[20%] sm:mt-[5%]   flex flex-col justify-center items-center'>
            <div className='flex justify-around text-sm sm:text-lg font-medium items-center bg-gray-900 border-2 border-b-0 text-white border-opacity-50 border-blue-800 rounded-t-lg w-[90%] sm:w-1/2 '>
                {/* send */}
                ...
                {/* Global Tx */}
                <div onClick={() => setRoute('global_tx')} className={`list-none cursor-pointer py-2 w-1/4 ${route == 'global_tx' ? "bg-black bg-opacity-60" : "bg-gray-900"} text-center rounded-tl-lg hover:bg-black hover:bg-opacity-60`}>
                    Global Tx
                </div>
            </div>
            {/* Screen */}
